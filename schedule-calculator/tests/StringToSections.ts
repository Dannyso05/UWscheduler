    ...
    static stringToWeeklySections(courseName: String, str: String, days: Days[]): Section[] {
        ...
        const sections: WeeklySection[] = rows.map((row) => {
            ...
            if (split.length == 14) {
                ...
                [courseCode, componentStr, sectionNumber, campusStr, loactionTaughtStr, num, assoc, rel1, rel2, num2, enrolCap, enrolTot, waitCap, waitTot, time] = split;
            } else {
                console.log("Split didn't give a permitted length. Gave " + split.length);
                const [courseCode, componentStr, sectionNumber, campusStr, loactionTaughtStr, num, assoc, rel1, rel2, enrolCap, enrolTot, waitCap, waitTot, time] = split;
            }
            ...
            let weeklySection = new WeeklySection(
                ...
                days,
                new Timeslot(new Time(this.convert12hTo24h(startTime.split(':')[0]), this.convert12hTo24h(startTime.split(':')[1])),
                    new Time(this.convert12hTo24h(endTime.split(':')[0]), this.convert12hTo24h(endTime.split(':')[1]))),
                parseInt(enrolCap),
                parseInt(enrolTot)
            );
    ...
    static stringToOneTimeSections(courseName: String, str: String, date: Date): Section[] {
        ...
        const sections: OneTimeSection[] = rows.map((row) => {
            ...
            let section = new OneTimeSection(
                ...
                parseInt(sectionNumber),
                date,
                new Timeslot(new Time(parseInt(startTime.split(':')[0]), parseInt(startTime.split(':')[1])), new Time(parseInt(endTime.split(':')[0]), parseInt(endTime.split(':')[1]))),
                parseInt(enrolCap),
                parseInt(enrolTot)
            );
